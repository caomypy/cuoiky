...
def read_products(keyword=None, from_price=None, to_price=None, is_latest=False): #tim kiem
...
def update_product(product_id, name, description, price, image, category): #update
    products = read_products()
    for p in products:
...
def read_products_by_cate_id2():
    return Category.query.get(2).products
...
def add_receipt(cart_products=[]):
    try:
        ...
        for p in cart_products:
            d = ReceiptDetail(product_id=p["id"], product_name=p["name"], receipt_id=r.id, unit_price=p["price"], quantity=p["quantity"])
            db.session.add(d)
